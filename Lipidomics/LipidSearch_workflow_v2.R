...
ggplot(comp_4, aes(y = Percentage, x = Group, fill = Lipid_Type)) +
  geom_bar(position = "dodge", stat = "identity")+ ggtitle("Top 4 Lipids by Lipid Type") +theme(plot.title=element_text(hjust=0.5)) + geom_text(aes(label=Percentage), position=position_dodge(width=0.9), vjust=-0.25) 

ggsave(here("Example_lipid_data","plots", "comp_by_lipids.svg"))
